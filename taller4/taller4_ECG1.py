...
print("=== TALLER 4: ALGORITMO PAN-TOMPKINS - ECG1 ===")
...
ecg_signal = data['ECG 1'].values  # Usamos ECG 1 (señal original, NO filtrada)
...
ax.set_title('Señal ECG Original - ECG1')
...
plt.suptitle('Algoritmo Pan-Tompkins - ECG1 (Todos los pasos)', fontsize=14)
plt.tight_layout()
plt.show(block=False)
...
ax.set_title(f'Detección de Ondas PQRST - ECG1 (Segmento {start_time}-{end_time}s)')
ax.set_xlabel('Tiempo (s)')
ax.set_ylabel('Amplitud (μV)')
ax.legend()
ax.grid(True, alpha=0.3)
...
if len(RR_intervals) > 0:
    ...
    plt.suptitle('Análisis de Variabilidad Cardíaca (HRV) - ECG1')
    plt.tight_layout()
    plt.show(block=False)
...
print("\n¡Análisis Pan-Tompkins completado para ECG1!")
plt.show()